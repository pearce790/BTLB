...
fitted_model <- btlb_mfmm(rankings=rankings,ratings=ratings,M=M,K_start=8,init=NULL,
                          delta_theta=c(5,0.25),delta_gamma=c(2,4),lambda=1.5,beta_mu=beta_mu,beta_nu=beta_nu,mu_0j=mu_0j,
                          T1=3000,T2=2,sigma_mu=0.2,sigma_nu=0.2,sigma_theta=3,sigma_gamma=0.5)
...
save(res,file=paste0("resCase1_",iteration_character,".Rdata"))
...
res <- list(model="Misspecified\nRatings",rater_effects=FALSE,ratings=ratings,rankings=rankings,Z=Z,I=I,K=K,seed=seed,
            ...
            results=fitted_model)
save(res,file=paste0("resCase2_",iteration_character,".Rdata"))
...
res <- list(model="Misspecified\nRankings",rater_effects=TRUE,ratings=ratings,rankings=rankings,Z=Z,I=I,K=K,seed=seed,
            sigma2_mu=sigma2_mu,sigma2_nu=sigma2_nu,mu=mu,nu=nu,theta=theta,alpha=alpha,
            results=fitted_model)
save(res,file=paste0("resCase3_",iteration_character,".Rdata"))
...
fitted_model <- btlb_mfmm_noratereffects(rankings=rankings, ratings=ratings, M=M, K_start=8, init=NULL, delta_theta=c(5,0.25),delta_gamma=c(2,4),lambda=1,
					beta_mu=beta_mu, mu_0j=mu_0j, T1=1000, T2=2, sigma_mu=0.2, sigma_theta=3, sigma_gamma=0.5)
fitted_model <- stephens_labelswitching(fitted_model,burn_prop=0.8,thin=1)
fitted_model$class <- NULL
res <- list(model="Case 4",rater_effects=FALSE,ratings=ratings,rankings=rankings,Z=Z,I=I,K=K,seed=seed,
            sigma2_mu=sigma2_mu,sigma2_nu=sigma2_nu,mu=mu,nu=nu,theta=theta,alpha=alpha,
            results=fitted_model)
save(res,file=paste0("resCase4_",iteration_character,".Rdata"))
