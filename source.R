...
library(label.switching) # implementing label switching algorithm of Stephens (2000)
library(rankrate) # general functions for rankings and ratings, plus Mallows-Binomial
...
btlb_map <- function(rankings,ratings,M,K,tol,verbose=FALSE,
  ...
  gamma_curr <- delta_gamma[1]/delta_gamma[2]
  sigma2_mu_curr <- beta_mu[2]/beta_mu[1]
  sigma2_nu_curr <- beta_nu[2]/beta_nu[1]
  ...
  while(continue){
    ...
    for(i in 1:I){
      for(k in 1:K){
        log_density[i,k] <- dbtlb(rankings=rankings_i_list[[i]],ratings=ratings[i,,drop=FALSE],p=expit(mu_curr[,k]+nu_curr[i]),theta=theta_curr[k],M=M,log=T)
      }
    }
    ...
    mutheta_new <- lapply(1:K,function(k){
      tryCatch({optim(par=c(mu_curr[,k],theta_curr[k]),
                      fn=function(param){
                        log_i <- unlist(lapply(1:I,function(i){
                          dbtlb(rankings=rankings_i_list[[i]],ratings=ratings[i,,drop=FALSE],p=expit(param[1:J]+nu_curr[i]),theta=param[J+1],M=M,log=TRUE)
                        }))
                      ...
                      method="L-BFGS-B",lower=c(mu_0j-10,1e-8),upper=c(mu_0j+10,qgamma(.999,delta_theta[1],delta_theta[2])),
        ...
        optim(par=c(mu_curr[,k],theta_curr[k]),
              ...
              method="L-BFGS-B",lower=c(mu_curr[,k]-1,max(.01,theta_curr[k]-5)),upper=c(mu_curr[,k]+1,theta_curr[k]+5),
              control=list(fnscale=-1))$par
      })})
    ...
    nu_new <- matrix(unlist(lapply(1:I,function(i){
      optimize(f=function(nu_new){dnorm(nu_new,mean=0,sd=sqrt(sigma2_nu_curr),log=T)+
          sum(z_hat[i,]*unlist(lapply(1:K,function(k){
            dbtlb(rankings=rankings_i_list[[i]],ratings=ratings[i,,drop=FALSE],p=expit(mu_curr[,k]+nu_new),theta=theta_curr[k],M=M,log=TRUE)
...
# Note 1: T1 and T2 correspond to B^{Gibbs} and B^{MH} as described in Algorithms 1 and 2.
# Note 2: sigma_mu, sigma_nu, sigma_theta, sigma_gamma control the standard deviation of the
#         proposal distribution for Metropolis-Hastings steps for mu, nu, theta, and gamma,
#         respectively.
