...
btlb_mfmm <- function(rankings,ratings,M,K_start,init,delta_theta,delta_gamma,lambda,beta_mu,beta_nu,mu_0j,
  ...
  log_dbtlb_nodatachecks <- function(rankings,ratings,assignments,p,worth,log_worth,J,R,M){
    I <- nrow(rankings)
    # rankings density - numerator
    log_numerator <- sum(log_worth[rankings],na.rm=T)
    # rankings density - denominator
    summed_worths_assignments <- apply(assignments,1,function(assignment){sum(worth[assignment])})
    summed_worths_uptocurrent <- t(rbind(rep(0,I),apply(rankings[,-J,drop=FALSE],1,function(ranking){cumsum(worth[ranking])})))
    for(i in 1:I){if(R[i]<J){summed_worths_uptocurrent[i,(R[i]+1):J] <- NA}}
    log_denominator <- sum(log(t(summed_worths_assignments - summed_worths_uptocurrent)),na.rm=T)
    # ratings density
    log_binomial <- sum(dbinom(t(ratings),M,p,log=T),na.rm=T)
    # overall density
    log_mass <- log_numerator - log_denominator + log_binomial
  ...
  for(t1 in 1:T1){
    ...
    log_probs <- matrix(unlist(mclapply(1:I,function(i){
      ...
      unlist(lapply(1:K_curr,function(k){
        log_dbtlb_nodatachecks(rankings=rankings[i,,drop=FALSE],ratings=ratings[i,,drop=FALSE],assignments=assignments[i,,drop=FALSE],
    ...
    mu_curr <- res_2bi[1:J,,drop=FALSE]
    ...
    res_2bii <- matrix(unlist(mclapply(1:I,function(i){
      ...
      curr_dens <- log_dbtlb_nodatachecks(rankings=rankings[i,,drop=FALSE],ratings=ratings[i,,drop=FALSE],assignments=assignments[i,,drop=FALSE],
                                          p=p_i,worth=exp(log_worths),log_worth=log_worths,J=J,R=R[i],M=M)+
      ...
      log_worths <- -theta_curr[Z_i]*p_i
      new_dens <- log_dbtlb_nodatachecks(rankings=rankings[i,,drop=FALSE],ratings=ratings[i,,drop=FALSE],assignments=assignments[i,,drop=FALSE],
                                         p=p_i,worth=exp(log_worths),log_worth=log_worths,J=J,R=R[i],M=M)+
